...
def insert_all_data():
    ...
    for brand in products['Brand'].dropna().unique():
        ...
        try:
            cursor.execute("IF NOT EXISTS (SELECT 1 FROM Brands WHERE BrandName = ?) INSERT INTO Brands (BrandName) VALUES (?)", brand, brand)
        except Exception as e:
            print("Error inserting brand:", brand, e)
    ...
    for _, row in products.iterrows():
        ...
        try:
            cursor.execute("""
                INSERT INTO Products (ProductID, ProductName, Category, BrandID, BrandName, Rating, ImageURL)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, row['ProductID'], row['ProductName'], row['Category'], brand_id, brand_name, rating, row['ImageURL'])
        except Exception as e:
